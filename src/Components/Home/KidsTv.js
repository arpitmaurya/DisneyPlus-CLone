...
import { selectKidsTv } from '../../Redux/Reducer/MovieReduer';
...
const KidsTv = () => {
    ...
    return (
        <>
            <Title>Disney for Kids</Title>
            <Carousel {...settings}>
                {
                    movies && movies.map((val, index) => (
                        <Wrap key={index}>
                            <NavLink className="slide-img" to={`/detail/${val.id}`}><img src={val.CardImg} alt={val.id} /></NavLink>
                        </Wrap>
                    ))
                }
            </Carousel>
